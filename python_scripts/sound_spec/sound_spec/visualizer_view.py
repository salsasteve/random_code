...
class VisualizerView:
    ...
    def init_bar_plot(self):
        ...
        self.bars = self.ax.bar(range(self.num_bins), np.zeros(self.num_bins), color=plt.cm.rainbow(np.linspace(0, 1, self.num_bins)))
        self.ax.set_ylim(0, 64)

        # Set x-axis ticks and labels
        x_positions = np.arange(self.num_bins)
        freq_labels = [f"{int(start_freq)}-{int(end_freq)}" for start_freq, end_freq in self.bin_config]
    ...
    def animate(self) -> FuncAnimation:
        interval = (self.model.fft_size / self.model.samplerate) * 1000  # in milliseconds
